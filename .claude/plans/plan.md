...
| 9   | `feat/carbon-theme`    | карбоновая тема, Framer Motion, весь экран, подборка «Популярное»          | подборка на пустом поиске: happy path + ошибка API                |
...
Подробности по факту (адаптер `CollectionItem → Movie` для переиспользования
`widgets/movie-grid` без изменений, находки ревью) — в
...
показывает подсказку; невалидный `?status=` не роняет страницу.
...
Не было в исходной карте этапов — визуальный рестайлинг по запросу
пользователя после этапа 8. Подробности по факту (выбор Framer Motion,
находка ревью про дублирующий `viewTransition`) — в
[`.claude/plans/stages/09-carbon-theme.md`](stages/09-carbon-theme.md).

- Единая тёмная карбоновая тема (токены в `app/styles/index.css`),
  красный акцент точечно.
- Framer Motion: переходы между страницами (`AnimatePresence` в
  `root-layout.tsx`), плавная смена статуса в `CollectionButton`,
  stagger-появление карточек, скользящее подчёркивание активной
  вкладки/ссылки.
- `whileHover`/`whileTap`/`onTap`/`drag` не используются нигде — несовместимы
  с `jest-fixed-jsdom` в этом проекте (см. «Архитектурные решения» в
  `CLAUDE.md`); hover/tap-фидбек — на CSS.
- Раскладка на всю ширину экрана (убран фиксированный `#root` от шаблона
  Vite); сетка результатов поиска — 5 колонок, крупнее постеры; поле
  поиска — фиксированной ширины, не на весь экран.
- Подборка «Популярное» на пустом поиске (`pages/search`), по TDD —
  курируемый список известных фильмов, не динамические «новинки»:
  `GET /movie` у poiskkino.dev не даёт быстрой сортировки по
  популярности/дате премьеры (см. «Архитектурные решения» в
  `CLAUDE.md`).
