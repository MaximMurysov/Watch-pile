...
- [x] Ревью субагентом `code-reviewer`: найден и исправлен баг [важно] —
      ...
      Zod-схемы
- [x] Найдено на ручной проверке в реальном браузере (не ревью, не
      юнит-тестами): незавершённый тег (без Enter/кнопки «Добавить
      тег») молча терялся при сабмите. Первая попытка чинить (`onBlur`
      на поле черновика) оказалась хуже — `blur` наступает между
      `mousedown`/`mouseup` клика по «Сохранить заметку», добавление
      чипа сдвигает вёрстку, и клик промахивается мимо кнопки, сабмит
      не срабатывает вовсе. Исправлено: `TagsField` отдаёт
      `commitDraftTag()` наружу через `forwardRef`/`useImperativeHandle`,
      `RateMovieForm` коммитит черновик тега внутри обработчика
      `submit`, до вызова `handleSubmit`, а не по `blur`. Подтверждено
      headless Playwright на реальной `/movie/:id` (jsdom-тесты сам
      промах клика не ловят — добавлен regression-тест на итоговое
      поведение «тег есть в сохранённой заметке», подробности в
      `stages/06-rate-movie.md`)
- [x] Чек-лист PR (`typecheck`/`lint`/`lint:fsd`/`test`/`build` — все
      зелёные, 25 suites / 89 tests)
- [ ] PR → merge → удаление ветки

## Этап 7 — Страница коллекции (`feat/collection-page`) — ✅ сделано

Подробности по факту (компромиссы адаптера `CollectionItem → Movie`,
находки ревью) — в
[`.claude/plans/stages/07-collection-page.md`](./stages/07-collection-page.md).

- [x] `pages/collection` — вкладки «хочу»/«посмотрел», статус в URL
- [x] Пустое состояние со ссылкой на поиск
- [x] Тест: переключение вкладки меняет список и URL
- [x] Тест: пустая коллекция → подсказка
- [x] Доп. тест (не было в исходном плане, покрывает пункт ревью-чеклиста
      про URL-параметры): невалидный `?status=` не роняет страницу
- [x] `pages/collection/lib/collection-item-to-movie` — адаптер записи
      коллекции к форме `Movie`, нужен был, чтобы переиспользовать
      `widgets/movie-grid` без изменения его пропсов (не было явно в
      исходном плане, обнаружено по факту работы)
- [x] `steiger.config.js` — снято временное исключение
      `fsd/insignificant-slice` для `widgets/movie-grid` (второй
      потребитель подключён, как и планировалось)
- [x] Ревью субагентом `code-reviewer`: критичных/важных замечаний нет,
      2 [мелочь] осознанно оставлены без изменений
- [x] Чек-лист PR (typecheck/lint/lint:fsd/test/build — все зелёные)
...
- [x] Страница 404 (`pages/not-found`, маршрут `path: "*"`) и
      `errorElement` роутера (`app/router/route-error-boundary.tsx`)
      — на отдельном вложенном route-объекте без своего `element`, а
      не на маршруте с `RootLayout` (найдено на ревью, см. ниже)
- [x] Скелетоны вместо спиннеров: `MovieCardSkeleton` в
      `widgets/movie-grid`, `MovieDetailsSkeleton` в `pages/movie`;
      `shared/ui/spinner.*` удалён как неиспользуемый
- [x] a11y: `:focus-visible` в `app/styles/index.css`, `aria-live` на
      обёртке `widgets/movie-grid` (без дублирующего `role="status"`
      внутри — найдено на ревью), `lang="ru"` в `index.html`, хук
      `useDocumentTitle` на всех страницах
- [x] README и CLAUDE.md приведены в соответствие с финальным
      поведением: раздел «Архитектурные решения» дополнен пунктами,
      фактически принятыми ещё на этапах 3/5 («страница владеет
      данными, виджет презентационный», заметка внутри
      `collection-item`, схема persist коллекции), но не
      зафиксированными вовремя
- [x] Отклонение от исходного плана этапа: под новое поведение
      (`useDocumentTitle`, `pages/not-found`) по согласованию с
      пользователем написаны минимальные тесты (TDD, red → green),
      хотя в таблице этапов `plan.md` для этапа 8 изначально стояло
      «без новых [тестов]»
- [x] Ревью субагентом `code-reviewer` (два прохода): первый нашёл
      [важно] — `errorElement` висел на одном route-объекте с
      `RootLayout`, из-за чего ошибка рендера страницы стирала Header
      вместе с содержимым; и 2 [мелочь] — дублирующий `role="status"`
      внутри уже живущего в `aria-live` контейнера, заголовок вкладки
      «Фильм» вместо «Фильм не найден» при 404/невалидном id. Все три
      исправлены и закреплены тестами (`route-error-boundary.test.tsx`
      проверяет, что layout не пропадает при ошибке; `movie-page.test.tsx`
      проверяет `document.title` на 404/невалидном id). Второй проход
      подтвердил исправления, нашёл дополнительную мелочь (нейминг
      константы `LOADING_TITLE` → `DEFAULT_MOVIE_TITLE`) — тоже
      исправлено
- [x] Чек-лист PR (`typecheck`/`lint`/`lint:fsd`/`test`/`build` —
      все зелёные, 31 suite / 101 test)
- [x] Ручная проверка в браузере (`pnpm dev` + Playwright, до правок
      ревью, поведение не изменилось): переход на несуществующий путь
      → страница 404 с рабочей ссылкой на поиск, header на месте;
      `<title>` меняется на `/`, `/movie/:id`, `/collection`;
      Tab-навигация показывает видимый фокус (`outline: 2px solid
      var(--accent)`); скелетон из 8 карточек корректно отображается
      во время загрузки поиска (искусственно задержан ответ API через
...
- [x] `#root` на всю ширину экрана (без фиксированных `1126px` и
      `border-inline` от шаблона Vite) — по запросу пользователя
- [x] Подборка «Популярное» на пустом поиске (`pages/search`) — по TDD;
      курируемый список `FEATURED_MOVIE_IDS` (10 id, каждый проверен
      через реальный `GET /movie/{id}`), не динамические «новинки»:
      сортировка/фильтр `GET /movie` по рейтингу/голосам/премьере у
      poiskkino.dev стабильно уходит в таймаут, единственная быстрая
      сортировка (`year`) отдаёт малоизвестные записи, а не
      блокбастеры — решение зафиксировано в CLAUDE.md
- [x] Сетка результатов поиска и подборки — 5 колонок (крупнее
      постеры), поле поиска — `max-width`, не на всю ширину экрана;
      общая сетка вынесена в `shared/ui/movie-card-grid.module.css`
      (CSS Modules `composes`), не дублируется
- [x] Чек-лист PR (`typecheck`/`lint`/`lint:fsd`/`test`/`build` —
      все зелёные, 31 suite / 103 test)
- [x] Ревью субагентом `code-reviewer` — два прохода. Первый (карбон +
      framer-motion): критичных замечаний нет; 1 [важно] (дублирующий
      `viewTransition` у `Link`/`NavLink` поверх `AnimatePresence`)
      исправлено; 3 [мелочь] осознанно оставлены. Второй (весь экран +
      подборка + 5 колонок): критичных нет; 3 [важно] исправлены —
      дублирование CSS сетки вынесено в `shared/ui`, добавлен
      `aria-live` в `FeaturedMovies`, добавлен стаггер запросов
      подборки (`FEATURED_CARD_STAGGER_MS`); 2 [мелочь] осознанно
