...
- поиск фильмов с пагинацией, состояние поиска хранится в URL; пока
  запрос не введён — подборка «Популярное» из известных фильмов, чтобы
...
- карбоновая тёмная тема с точечным красным акцентом; плавные переходы
  между страницами и при изменении статуса в коллекции (Framer Motion,
